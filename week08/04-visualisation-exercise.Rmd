...
yrng <- range(unemp$unemp, na.rm = T)
xrng <- range(unemp$date, na.rm = T)
...
p2 <- p1 +
  geom_rect(data = pres, 
            ...
            alpha = 1/5) +
  scale_fill_manual(values = c("blue", "red"), 0.2) +
  scale_x_date(limits = xrng, expand = c(0,0)) +
  geom_text(pres,
            aes(start, yrng[2], label = name))
...
p3 <- p2 +
  geom_vline(aes(colour="black", linetype = "longdash")) +
  geom_segment(pres, aes(x = start, y = y1, xend = end, yend = y2, colour = "segment")
